...
@interface DPChromecastMediaContext ()

- (void) cache;

+ (DPChromecastMediaContext *) findContextWithMediaId:(NSString *)mediaId;
...
- (void) cache
{
    if (![DPChromecastMediaContext findContextWithMediaId:_mediaId]) {
    ...
    if (instance) {
        ...
        if ((type & MPMediaTypeAnyVideo) != 0) {
            ...
            [NSString stringWithFormat:@"%@:%llu",
             DPChromecastMediaIdSchemeIPodMovie, persistentId.unsignedLongLongValue];
        } else if ((type & MPMediaTypeAnyAudio) != 0) {
            instance.isAudio = YES;
            instance.type = @"Audio";
            instance.mediaId =
            [NSString stringWithFormat:@"%@:%llu",
             DPChromecastMediaIdSchemeIPodAudio, persistentId.unsignedLongLongValue];
        } else {
            // ????
            return nil;
        }
